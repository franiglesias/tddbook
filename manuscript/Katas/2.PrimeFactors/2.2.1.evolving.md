...
[The Transformation Priority Premise](http://blog.cleancoder.com/uncle-bob/2013/05/27/TheTransformationPriorityPremise.html) es un artículo que plantea un framework útil en este sentido. Partiendo de la idea de que a medida que los tests son más específicos el código se vuelve más general, propone una secuencia del tipo de transformaciones que podemos aplicar cada vez que estamos en fase de implementación, en la transición de rojo a verde.
...
Al tratarse de n nuevo símbolo lo trataremos de manera especial:
...
Si observamos el código de producción observamos estructuras similares, pero no está del todo claro un patrón completo. Quizá necesitamos más información. Vamos al caso siguiente:
...
Esto sucede porque no estamos acumulando la notación romana en la variable que vamos a retornar, por lo que en algunos lugares *machacamos* el resultado existente. Hagamos el cambio:
...
El problema es que necesitamos *quitar* todos los 10 contenidos en el número.
...
¿Podríamos usar while en todos los casos? Ahora que estamos en verde, podríamos probar a cambiar todas las condiciones de `if` a `while`. Y los tests demuestran que es posible hacerlo:
...
Es interesante porque cada vez las estructuras son más parecidas. Probemos ahora a cambiar los casos en que usamos una igualdad para ver si podemos usar `>=` en su lugar.
...
Los test siguen pasando, indicación de que nuestro refactor es correcto. De hecho, no tendríamos fallos hasta llegar al número 39. Algo esperable, porque se introduce un símbolo nuevo:
...
Pequeñas transformaciones del código de producción pueden dar lugar a cambios grandes de comportamiento, aunque para eso necesitaremos también dedicar tiempo al refactor.
