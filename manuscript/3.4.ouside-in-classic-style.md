...
El test fallará porque no coinciden la lista generada con la esperada. Para hacerlo pasar necesitaremos reintroducir el repositorio, de modo que podamos recuperar las tareas guardadas.
...
def todo_application
  @task_repository = TaskRepository.new MemoryStorage.new
  @add_task_handler = AddTaskHandler.new @task_repository
  TodoListApp.new @add_task_handler, @task_repository
end
...
Personalmente creo que me interesa abordar antes este último. Puestos a devolver una respuesta hardcoded, puedo empezar con la transformación desde el objeto Task y luego ya continuaré el desarrollo de TaskRepository.
...
Ruby es bastante conciso, aún así, voy a hacer algún refactor en el test de aceptación:
...
Así que vamos a empezar a añadir código de producción hasta lograr que el test pase. Por supuesto, el primero problema es que no hay una ruta ni controlador asociado.
...
Y este error ya es exactamente que la tarea completada aparece sin marcar, que es exactamente donde queremos estar.
...
Introducimos la nueva clase, que simplemente usa el mismo código que ya está testeado.
...
Y el test sigue pasando, como era de esperar. Así que podemos eliminar el método que extraído antes. Después tendremos que cambiar la construcción para inyectar el caso de uso. Pero vamos por partes:
